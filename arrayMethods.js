...
// CODE HERE
// let initialVal = 0;
const totalPopulation = populations.reduce((pv, cv) => pv + cv);
...
// CODE HERE
const orderTotals = () => {
  return orders.map(order => order.price * (1 + order.tax))
}
...
sumBobsTotal = () => {
  return purchases.filter(purchase => purchase.owner === "Bob").reduce((pv, cv) => pv + cv.price, 0)
}